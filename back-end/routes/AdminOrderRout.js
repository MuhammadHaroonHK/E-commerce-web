...
router.put("/:id", protect, admin, async (req, res) => {
  try {
    ...
    if (order) {
      ...
      order.isDelivered =
        req.body.status === "Delevired" ? true : order.isDelivered;
      order.deliveredAt =
        req.body.status === "Delevired" ? Date.now() : order.deliveredAt;
    }

    const updatedStatus = await order.save();
    res
      .status(200)
      .json({ msg: "user updated successfully", order: updatedStatus });
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: "Server Error" });
  }
